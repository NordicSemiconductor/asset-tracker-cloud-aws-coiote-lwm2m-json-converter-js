...
describe('convertResource', () => {
	it.each([
		...
		"should convert prop '%s' from LwM2M Server object. Value: '%s' Expected: %s",
		(propId, value, expected) => {
			const LwM2MServerSchema = {
				type: 'object',
			...
			expect(
				convertResource(LwM2MServerSchema as any, propId, value as any),
			).toEqual(expected)
		},
	)
})
